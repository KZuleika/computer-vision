...
def plotHistogram(imgOriginal,numfila):
    channels = cv2.split(imgOriginal)
...
def ecualizacion_histograma_rgb(img_rgb):
    out = img.copy()
    #print(img[:,:,0].shape)
    out[:,:,0] = ecualizacion_histograma_gray(img_rgb[:,:,0])
    out[:,:,1] = ecualizacion_histograma_gray(img_rgb[:,:,1])
...
for i in range(len(imgtxt)):
    ...
    img_ecualizada = ecualizacion_histograma_rgb(img)
    ...
    plt.savefig(imgtxt[i]+"Hist"+".png", dpi = 600)
    plt.show()
