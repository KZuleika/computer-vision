...
#cerradura
cerradura = []
cerradura.append(cv2.morphologyEx(imgBinarizada[2],cv2.MORPH_CLOSE,kernel[0]))
cerradura.append(cv2.morphologyEx(imgBinarizada[2],cv2.MORPH_CLOSE,kernel[1]))
cerradura.append(cv2.morphologyEx(imgBinarizada[2],cv2.MORPH_CLOSE,kernel[2]))

plt.subplot(221),plt.imshow(imgBinarizada[2], cmap = 'gray')
plt.title('Imagen Original'), plt.xticks([]), plt.yticks([])

plt.subplot(222),plt.imshow(cerradura[0], cmap = 'gray')
plt.title('MORPH_RECT'), plt.xticks([]), plt.yticks([])

plt.subplot(223),plt.imshow(cerradura[1], cmap = 'gray')
plt.title('MORPH_CROSS'), plt.xticks([]), plt.yticks([])

plt.subplot(224),plt.imshow(cerradura[2], cmap = 'gray')
plt.title('MORPH_ELIPSE'), plt.xticks([]), plt.yticks([])

plt.savefig("cerradura.png", dpi = 600)
plt.show()
