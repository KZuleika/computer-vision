...
#apertura
apertura = []
apertura.append(cv2.morphologyEx(imgBinarizada[1], cv2.MORPH_OPEN,kernel[0]))
apertura.append(cv2.morphologyEx(imgBinarizada[1], cv2.MORPH_OPEN,kernel[1]))
apertura.append(cv2.morphologyEx(imgBinarizada[1], cv2.MORPH_OPEN,kernel[2]))

plt.subplot(221),plt.imshow(imgBinarizada[1], cmap = 'gray')
plt.title('Imagen Original'), plt.xticks([]), plt.yticks([])

plt.subplot(222),plt.imshow(apertura[0], cmap = 'gray')
plt.title('MORPH_RECT'), plt.xticks([]), plt.yticks([])

plt.subplot(223),plt.imshow(apertura[1], cmap = 'gray')
plt.title('MORPH_CROSS'), plt.xticks([]), plt.yticks([])

plt.subplot(224),plt.imshow(apertura[2], cmap = 'gray')
plt.title('MORPH_ELIPSE'), plt.xticks([]), plt.yticks([])

plt.savefig("apertura.png", dpi = 600)
plt.show()
