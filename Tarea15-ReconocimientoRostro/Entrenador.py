import cv2
import os
import numpy as np

def EigenFace_train(facesData, labels):
    # Métodos para entrenar el reconocedor
    face_recognizer = cv2.face.EigenFaceRecognizer_create()

    # Entrenando el reconocedor de rostros
    print("Entrenando EigenFaces ...")
    face_recognizer.train(facesData, np.array(labels))
    print("Entrenamientos finalizados ...")

    # Almacenando el modelo obtenido
    face_recognizer.write('modeloEigenFace.xml')
    print("Modelos almacenados...")

def FisherFace_train(facesData, labels):
    face_recognizer = cv2.face.FisherFaceRecognizer_create()

    print("Entrenando FisherFaces ...")
    face_recognizer.train(facesData, np.array(labels))
    print("Entrenamientos finalizados ...")

    face_recognizer.write('modeloFisherFace.xml')
    print("Modelos almacenados...")

def LBPHFace_train(facesData, labels):
    face_recognizer= cv2.face.LBPHFaceRecognizer_create()

    print("Entrenando LBPHFaces ...")
    face_recognizer.train(facesData, np.array(labels))
    print("Entrenamientos finalizados ...")

    face_recognizer.write('modeloLBPHFace.xml')
    print("Modelos almacenados...")

dataPath = 'C:/Users/kzcoc/OneDrive - Universidad Autonoma de Yucatan/8SEMM/VisionPorComputadora/Tarea17-ReconocimientoRostro/Data' #Cambia a la ruta donde hayas almacenado Data
peopleList = os.listdir(dataPath)
print('Lista de personas: ', peopleList)
labels = []
facesData = []
label = 0
for nameDir in peopleList:
    personPath = dataPath + '/' + nameDir
    print('Leyendo las imágenes')
    for fileName in os.listdir(personPath):
        print('Rostros: ', nameDir + '/' + fileName)
        labels.append(label)
        facesData.append(cv2.imread(personPath+'/'+fileName,0))
        image = cv2.imread(personPath+'/'+fileName,0)
        cv2.imshow('image',image)
        cv2.waitKey(10)
    label = label + 1
print('labels= ',labels)
print('Número de etiquetas 0: ',np.count_nonzero(np.array(labels)==0))
print('Número de etiquetas 1: ',np.count_nonzero(np.array(labels)==1))
print('Número de etiquetas 2: ',np.count_nonzero(np.array(labels)==2))

EigenFace_train(facesData, labels)
FisherFace_train(facesData, labels)
LBPHFace_train(facesData, labels)

