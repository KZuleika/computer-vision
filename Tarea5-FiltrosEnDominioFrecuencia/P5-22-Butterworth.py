...
def buterrworth_filter(val):
    n = cv2.getTrackbarPos('orden','Butterworth')
    fcorte = cv2.getTrackbarPos('frec','Butterworth')
...
for i in range(0, len(imgNames)):
    ...
    cv2.createTrackbar('frec', winTitle,0,300, buterrworth_filter)
    cv2.createTrackbar('orden', winTitle,0,20, buterrworth_filter)
    cv2.waitKey(0)
