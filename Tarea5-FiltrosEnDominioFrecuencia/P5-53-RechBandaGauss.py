...
def pbanda_gauss_filter(val):
    ...
    for i in range(alto):
        for j in range (ancho):
            ...
            aux = d**2 / (2 * fcMax**2)
            pbaja1[i,j,:] = math.exp(-aux)
            aux = d**2 / (2 * fcMin**2)
            pbaja2[i,j,:] = math.exp(-aux)
...
for i in range(0, len(imgNames)):
    ...
    cv2.createTrackbar('fc1', winTitle,0,300, pbanda_gauss_filter)
    cv2.createTrackbar('fc2', winTitle,0,300, pbanda_gauss_filter)
    cv2.waitKey(0)
